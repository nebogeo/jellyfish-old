...
;           (write! (+ vertex 1026) c)
     ...
     ;(play-now (mul (adsr 0 0.1 0.2 2) 
     ;               (sine (add 440 (mul (adsr 1 2 1 2) (mul (saw 290) 3000))))))
       ...
       (loop (< vertex positions-end)         
         ...
         (cond
          ((> (swizzle yzz (read vertex)) 4)
           (play-now (mul (adsr 0 0.1 0 0) (white 40)))
...
(define (jelly-setup2)
  ...
  (with-primitive
   jelly
   (terrain-setup)
   (let ((p (compile-program 10000 prim-triangles 1 synthtest)))
     ;;(disassemble p)
     (jelly-compiled p)
     ))
